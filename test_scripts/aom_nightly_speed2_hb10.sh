...
awk '{print $9}' <$dlog &>>$test_dir/s2bd10output2.txt
echo | sed 's/(//' <$test_dir/s2bd10output2.txt &>>$test_dir/s2bd10output3.txt
awk '{ total += $1; count++ } END { print total/count }' $test_dir/s2bd10output3.txt &>>$test_dir/s2bd10sum.txt
dfps=`awk '{print $1}' < $test_dir/s2bd10sum.txt`
