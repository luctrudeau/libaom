...
awk '{print $9}' <$dlog &>>$test_dir/s3output2.txt
echo | sed 's/(//' <$test_dir/s3output2.txt &>>$test_dir/s3output3.txt
awk '{ total += $1; count++ } END { print total/count }' $test_dir/s3output3.txt &>>$test_dir/s3sum.txt
dfps=`awk '{print $1}' < $test_dir/s3sum.txt` 
...
echo "    <th style="color:blue" colspan=\"8\">AV1 SP3 BD8: $(basename $video), bitrate=$bitrate profile=$profile bit-depth=$bd frames=$frames speed=$speed" >> $log_path/$html_log_file
echo "  </tr>" >> $log_path/$html_log_file
