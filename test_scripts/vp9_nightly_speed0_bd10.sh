...
# Preassumption:
#  1) Assume all script files are in ~/Devtest/sandbox/libvpx/scripts
# Note:
...
script_dir=~/Devtest/sandbox/libvpx/scripts
root_dir=~/Devtest/vp9d

cd $root_dir/libvpx
commit=`git log --pretty=%h -1`

. $script_dir/video_nightly_bd10.sh
...
bstream=vp9_nightly_hbd_profile_$profile.$commit.webm
elog=vp9enc_nightly_log_hbd_p_$profile.$commit.txt
dlog=vp9dec_nightly_log_hbd_p_$profile.$commit.txt
